                ...
                <div className="space-y-6">
                    {items.map((item) => (
                        <div key={item.id} className="group relative flex gap-4 p-2 hover:bg-gray-50 rounded-xl transition-colors">
                            <div className="w-20 h-24 bg-gray-100 rounded-lg overflow-hidden shrink-0 cursor-pointer" onClick={() => handleAddToCart(item)}>
                                {/* Item might be just ID if loaded from lightweight API? 
                     Hopefully store has full object. 
                     Assuming full object based on Add logic. 
                 */}
